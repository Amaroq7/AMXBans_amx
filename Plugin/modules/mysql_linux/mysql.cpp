...
int sqlconnect(MYSQL *mysql, const char* sql_host, const char* sql_user, const char* sql_pass, const char* sql_db, char** sqlerror, int reconnect) {
	...
	mysql_init(mysql);
	my_bool mb_reconnect = reconnect;
	mysql_options(mysql, MYSQL_OPT_RECONNECT, &mb_reconnect);
...
// sql = mysql_connect(host[],user[],pass[],dbname[],error[],maxlength,reconnect) (7 param):
// - open connection
NATIVE(sql_connect) {
	int i;
...
NATIVE(sql_getfield) {
	...
	case 3: {
				*MF_GetAmxAddr(amx, params[3]) = (cell) ((float) atof(data));
				return 1;
	}
	case 4: {
