...
if(isset($_POST["edit_ban"]) && isset($_POST["bid"]) && $_SESSION["loggedin"]) {
	...
	if(!$error) {
		//save ban edit log
		$mysql->query("INSERT INTO `".$config->db_prefix."_bans_edit` (`bid`,`edit_time`,`admin_nick`,`edit_reason`) 
		...
		$mysql->query($edit_query." WHERE `bid`=".$bid 
				) or die ($mysql->error);
...
if(isset($_POST["del_ban_x"]) && isset($_POST["bid"]) && $_SESSION["loggedin"]) {
	...
	$query = $mysql_query("SELECT `id`,`demo_file` FROM `".$config->db_prefix."_files` WHERE `bid`=".$bid) or die ($mysql->error);
...
if(isset($_POST["del_comment_x"]) && isset($_POST["cid"]) && $_SESSION["loggedin"]) {
	$mysql->query("DELETE FROM `".$config->db_prefix."_comments` WHERE `id`=".(int)$_POST["cid"]." LIMIT 1") or die ($mysql->error);
...
if(isset($_POST["add_comment"]) && $bid) {
	...
	if(!$error) {
		$mysql->query("INSERT INTO `".$config->db_prefix."_comments` (`name`,`comment`,`email`,`addr`,`date`,`bid`) 
...
if(isset($_POST["edit_comment"]) && isset($_POST["cid"]) && $_SESSION["loggedin"]) {
	if(!$error) {
		//save it to db
		$mysql->query("UPDATE `".$config->db_prefix."_comments` SET `comment`='".$comment."',`name`='".$name."',`email`='".$email."' WHERE `id`=".(int)$_POST["cid"] 
				) or die ($mysql->error);
...
if(isset($_POST["edit_demo"]) && isset($_POST["did"]) && $_SESSION["loggedin"]) {
	...
	if(!$error) {
		//save it to db
		$mysql->query("UPDATE `".$config->db_prefix."_files` SET `comment`='".$comment."',`name`='".$name."',`email`='".$email."' WHERE `id`=".(int)$_POST["did"] 
				) or die ($mysql->error);
...
if(isset($_POST["add_demo"]) && isset($_FILES['filename']['tmp_name']) && ($config->demo_all || $_SESSION["loggedin"])) {
	...
	if(!$error) {
		//save it to db
		$mysql->query("INSERT INTO `".$config->db_prefix."_files` (`upload_time`,`down_count`,`bid`,`demo_file`,`demo_real`,`comment`,`name`,`email`,`file_size`,`addr`) 
...
if(isset($_POST["down_demo_x"]) && isset($_POST["did"])) {
	...
	if(!$error) {
		//add download count
		$mysql->query("UPDATE `".$config->db_prefix."_files` SET `down_count`=`down_count`+1 WHERE `id`=".(int)$_POST["did"]) or die ($mysql->error);
