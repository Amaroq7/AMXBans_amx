...
function menu_change_pos($mid,$pos,$pos_new) {
	...
	$mysql->query("UPDATE `".$config->db_prefix."_usermenu` SET `pos`=0 WHERE `id`=".$mid." LIMIT 1") or die ($mysql->error);
	
	if($pos==$pos_new-1 || $pos==$pos_new+1) {
		//pos_new one lower or higher (arrows)
		$mysql->query("UPDATE `".$config->db_prefix."_usermenu` SET `pos`=`pos`".(($pos_new < $pos)?"+":"-")."1 
	...
	$mysql->query("UPDATE `".$config->db_prefix."_usermenu` SET `pos`=".$pos_new." WHERE `id`= ".$mid." LIMIT 1") or die ($mysql->error);
...
if(isset($_POST["del"]) && $_SESSION["loggedin"]) {
	$mysql->query("DELETE FROM `".$config->db_prefix."_usermenu` WHERE `id`=".$mid." LIMIT 1") or die ($mysql->error);
...
if(isset($_POST["new"]) && $_SESSION["loggedin"]) {
	$mysql->query("INSERT INTO `".$config->db_prefix."_usermenu` 
...
if(isset($_POST["save"]) && $_SESSION["loggedin"]) {
		...
		$mysql->query("UPDATE `".$config->db_prefix."_usermenu` SET 
