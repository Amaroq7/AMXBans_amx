...
*/
	...
	if(isset($_POST["new"]) && $_SESSION["loggedin"]) {
		...
		$level_count=(int)$count[0];
		$mysql->query("INSERT INTO `".$config->db_prefix."_levels` (`level`) VALUES (".($level_count+1).")") or die ($mysql->error);
	...
	if(isset($_POST["del"]) && $_SESSION["loggedin"]) {
		...
		//if not, delete level
			$mysql->query("DELETE FROM `".$config->db_prefix."_levels` WHERE `level`=".$lid." LIMIT 1") or die ($mysql->error);
	...
	if(isset($_POST["save"]) && $_SESSION["loggedin"]) {
		...
		$mysql->query("UPDATE `".$config->db_prefix."_levels` SET 
		...
		$mysql->query("UPDATE `".$config->db_prefix."_webadmins` SET `logcode`='' WHERE `level`=".$lid) or die ($mysql->error);
