...
if(isset($_POST["delall"]) && $_SESSION["loggedin"]) {
	$mysql->query("DELETE FROM `".$config->db_prefix."_logs`") or die ($mysql->error);
	$user_msg="_LOGDELETED";
	log_to_db("Logs del","deleted all logs");
}
if(isset($_POST["delolder"]) && $_SESSION["loggedin"]) {
	$days=(int)$_POST["days"];
		$mysql->query("DELETE FROM `".$config->db_prefix."_logs` WHERE UNIX_TIMESTAMP(now()) - timestamp > ".($days*84600)) or die ($mysql->error);
	$user_msg="_LOGDELETED";
