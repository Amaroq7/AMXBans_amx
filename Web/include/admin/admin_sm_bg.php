...
*/
	...
	if(isset($_POST["newset"]) && $_SESSION["loggedin"]) {
		...
		if(!$user_msg) {
			$mysql->query("INSERT INTO `".$config->db_prefix."_reasons_set` (`setname`)	VALUES ('".$setname."')") or die ($mysql->error);
	...
	if(isset($_POST["newreason"]) && $_SESSION["loggedin"]) {
		...
		if(!$user_msg) {
			$mysql->query("INSERT INTO `".$config->db_prefix."_reasons` (`reason`,`static_bantime`)	VALUES ('".$reason."',".$time.")") or die ($mysql->error);
	...
	if(isset($_POST["delset"]) && $_SESSION["loggedin"]) {
		...
		//delete the set
		$mysql->query("DELETE FROM `".$config->db_prefix."_reasons_set` WHERE `id`=".$rsid." LIMIT 1") or die ($mysql->error);
		//delete all reasons for set
		$mysql->query("DELETE FROM `".$config->db_prefix."_reasons_to_set` WHERE `setid`=".$rsid) or die ($mysql->error);
		...
		if(!$user_msg) {
			...
			if (isset($_POST["aktiv"])) {
				foreach ($_POST["aktiv"] as $k => $v) {
					$mysql->query("INSERT INTO `".$config->db_prefix."_reasons_to_set` (`setid`,`reasonid`) VALUES (".$rsid.",".$v.")") or die ($mysql->error);
				}
			}
			$mysql->query("UPDATE `".$config->db_prefix."_reasons_set` SET `setname`='".$setname."' WHERE `id`=".$rsid." LIMIT 1") or die ($mysql->error);
	...
	if(isset($_POST["reasondel"]) && $_SESSION["loggedin"]) {
		...
		$mysql->query("DELETE FROM `".$config->db_prefix."_reasons` WHERE `id`=".$rid." LIMIT 1") or die ($mysql->error);
		$mysql->query("DELETE FROM `".$config->db_prefix."_reasons_to_set` WHERE `reasonid`=".$rid) or die ($mysql->error);
	...
	if(isset($_POST["reasonsave"]) && $_SESSION["loggedin"]) {
		...
		if(!validate_value($reason,"name",$error,1,99,"REASON")) $user_msg=$error;
		if(!$user_msg) {
			$mysql->query("UPDATE `".$config->db_prefix."_reasons` SET `reason`='".$reason."',`static_bantime`=".$time." WHERE `id`=".$rid." LIMIT 1") or die ($mysql->error);
